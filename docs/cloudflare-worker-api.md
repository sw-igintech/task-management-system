...
| Var | Type | Production value | Notes |
|---|---|---|---|
| `EMAIL_ENABLED` | var | `"true"` | Must be exactly `"true"` to send (staging stays `"false"`). |
| `EMAIL_FROM` | var | `Task Manager <notifications@task-notification.xyz>` | Verified Resend sender. |
| `EMAIL_REPLY_TO` | var | `sw@igintech.com` | Optional Reply-To; added to the payload when set. |
...
4. Email notifications (Resend) from the Worker — **enabled in production**
